...
C     .. Local Scalars ..
     ...
     +        IYEAR,IYES,LDUM,LUN,LUNIN,LUNOUT,NREC
      CHARACTER ERRSTR*30,HANDLE* (LSTR),NAME1* (LSTR),NAME2* (LSTR),
...
C     .. External Subroutines ..
     ...
     +         UIDATE,UISATT,URENAM,USTIME,UTIME,CCPRCS,CCPDPN
C     ..
...
C---- URENAM
C
      CALL URENAM(NAME1,NAME2,ISTAT)
      ERRSTR = 'OK'
      IF (ISTAT.NE.0) CALL UGERR(ISTAT,ERSTR)
      WRITE (LUNOUT,FMT=6002) ' URENAM',ERRSTR,'Check rename status'
      CALL CUNLINK (NAME2)
C
C---- UGERR
C
