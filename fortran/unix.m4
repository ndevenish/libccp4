...
C     
      JUNK = ETIME(TARRAY)
      IF (JUNK.LT.0.) THEN
        WRITE(6,'(A)') ' ERROR in s/r GETELAPSED calling ETIME'
        STOP
      END IF
      CALL USTIME(ELAPS)
      ELAPS = ELAPS - START
...
ifelse(_ifc,1,
[      VALTMP = ' '
      CALL GETENV(NAMENV(:LENSTR(NAMENV)),VALTMP)
      ILEN = LENSTR(VALENV)
c     will it fit in?
      IF (ILEN.LE.512) THEN
        VALENV = VALTMP(:LENSTR(VALTMP))
      ELSE
        VALENV(:ILEN) = VALTMP(:MIN(ILEN,LENSTR(VALTMP)))
      END IF],
...
C          Convex uses STIME (), others seem to use TIME () or LTIME.
C
C Input:     none
C
C Output:    ISEC
C
C Arguments: INTEGER ISEC
C
C Usage:     CALL USTIME(ISEC)
C
      INTEGER ISEC
C
ifelse(_convex,1,
