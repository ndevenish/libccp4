...
c  40                                            CONTINUE
			nvlen = ((p4-p0)+p3)/p4
 		call scopy(nvlen, x(p0), p4, t2, 1)
 		call scopy(nvlen, x(p0+delta), p4, x(p0), p4)
 		call scopy(nvlen, t2, 1, x(p0+delta), p4)
 		call scopy(nvlen, y(p0), p4, t2, 1)
 		call scopy(nvlen, y(p0+delta), p4, y(p0), p4)
 		call scopy(nvlen, t2, 1, y(p0+delta), p4)
   50                                            CONTINUE
                                                 END IF
   60                                            CONTINUE
   70                                            CONTINUE
...
cvd$  cncall
          DO 30 I = 2,NOVER2
		call realft_p(odd, even, n, dim, i)
   30         CONTINUE
      END IF
C
      IF (N.GE.1) THEN
...
C
C
      recursive SUBROUTINE realft_p(EVEN,ODD,N,DIM, i)
C      ======================================
C
C     .. Scalar Arguments ..
      INTEGER           i, N
...
C     .. Local Scalars ..
      REAL              A,ANGLE,B,C,CO,D,E,F,SI,TWON,TWOPI
      INTEGER           D2,D3,D4,D5,I0,I1,I2,J,K,L,NOVER2,NT
C     ..
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC         COS,REAL,SIN
